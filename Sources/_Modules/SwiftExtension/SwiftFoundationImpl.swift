...
extension UserDefaults {
  // 內部標記，看輸入法是否處於測試模式。[Unsafe]
  public static let pendingUnitTests: Mutex<Bool> = .init(false)

  public nonisolated(unsafe) static let unitTests = UserDefaults(suiteName: "UnitTests")

  public static var current: UserDefaults {
    pendingUnitTests.withLock {
      $0 ? .unitTests ?? .standard : .standard
    }
  }
}
...
public func asyncOnMain(
  execute work: @MainActor @escaping @convention(block) () -> ()
) {
