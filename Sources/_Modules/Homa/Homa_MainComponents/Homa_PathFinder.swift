...
extension Homa {
  final class PathFinder {
    ...
    init(config: Homa.Config, assembledSentence: inout [Homa.GramInPath]) {
      ...
      var parent = [GramTrio?](repeating: nil, count: keyCount + 1)
        ...
        for (length, nextNode) in config.segments[i] {
          ...
          if newScore > dp[nextPos] {
            ...
            parent[nextPos] = (
              ...
              isExplicit: nextNode.isExplicitlyOverridden
            )
          }
        }
      }
      ...
      while currentPos > 0 {
        ...
        resultReversed.append(
          .init(
            gram: gram,
            isOverridden: parentInfo.isOverridden,
            isExplicitlyOverridden: parentInfo.isExplicit
          )
        )
    ...
    private typealias GramTrio = (gram: Homa.Gram?, isOverridden: Bool, isExplicit: Bool)
  }
}
