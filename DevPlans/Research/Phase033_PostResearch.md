...
Phase 33 先完成了 Phase 32 調研報告（`Phase032_PostResearch.md`）所規劃的**階段 A1：引擎替換 Megrez → Homa**，並於 2026-04-20 補完了 **階段 A2：InputHandler 精簡** 與 **階段 B：`LMPerceptionOverride` → `LXPerceptor` / `LX_Perceptor` 對齊**。其後同日亦完成了 `vChewing-OSX-Legacy` 的對位 transplant、文件同步與 mirror audit 收尾。
...
- **現況**：這個風險已被結構性消除；其後 follow-up 也補完了 Homa revolver / consolidator 與測試端的 identity 對位缺陷，因此 `Assembler_TestCandidateRevolvementWithConsolidation` 已恢復通過。
- **範圍**：`vChewing-macOS` + `vChewing-LibVanguard` 同步修復。
...
- 之所以現在能安全移除該 fallback，是因為 follow-up 已把 Homa 端的兩個根因一起補齊：`Assembler_TestCandidateRevolvementWithConsolidation` 的 candidate identity 對位改為依 `pair + candidate span` 解析；`revolveCandidate()` / `consolidateCandidateCursorContext()` 也改為以「目標候選真正穩定成為 logical cursor 上的 explicit node」為成功條件，並用候選實際跨度計算 consolidation 邊界。
