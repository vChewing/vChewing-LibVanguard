...
> 最後更新：2026-05-16（Phase 74: i18n key 全遷移——UserDef Mirror 自動遷移 + 非 UserDef 顯式映射表遷移，263 項映射 21 命名空間，雙倉庫 542 IME + 37 Installer keys 全轉換，零 bare 殘留。含 KIMI 稽核修補。）
...
- Phase 48 已完成：**LMCoreEX 前綴部分匹配**。在 `LMCoreEX` 之上疊加輕量前綴索引 `sortedKeys: [String]`（由 `rangeMap.keys.sorted()` 產生），新增 `keys(matchingPrefix:)`（二分搜尋 sortedKeys + 線性掃描 temporaryMap）與 `unigramsFor(keyPrefix:)`（對每個匹配 key 自動推導正確 `keyArray` 後走 exact-match 路徑）。`LMInstantiator.unigramsFor(keyArray:)` 新增 `partiallyMatch: Bool` 過載，預設 `false` 以維持既有 exact-match 行為；`LookupHub.grams(for:)` 改傳 `partiallyMatch: lmi.config.partialMatchEnabled`，使使用者語彙（`lmUserPhrases`）在開啟 partial match 時也能回傳前綴匹配結果。單漢字權重上限邏輯修正為僅對單音節匹配結果生效（避免多音節前綴匹配結果被錯誤壓分）。`lmUserSymbols` 與 `lmFiltered` 維持 exact-match 不變。**Review follow-up**：`LMInstantiator.unigramsFor(keyArray:partiallyMatch:)` 的 LRU 快取鍵加入模式維度（`PM0/PM1`）以隔離 exact / partial 查詢結果；`LMCoreEX.keys(matchingPrefix:)` 新增 key 去重並將 `temporaryMap.keys` 排序，修復重複回傳與順序抖動。新增 `testPartialAndExactQueriesDoNotShareCacheEntry`、`testPrefixMatchingDeduplicatesOverlappingMainAndTemporaryKeys` 回歸測試。已同步至 `vChewing-macOS` 與 `vChewing-OSX-Legacy`；LangModelAssembly focused tests 16/16、Legacy `make debug-core` 均通過。
...
> ⚠️ **注意**: 本文檔需要定期更新以反映最新程式碼狀態。最後更新：Phase 70（2026-05-15）。
