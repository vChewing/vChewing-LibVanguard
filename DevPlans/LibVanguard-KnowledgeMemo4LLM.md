...
> 最後更新：2026-07-24（Phase 114 完成——IMK dispatch 全面改用 class-level static blocks，消滅 per-instance block ivars、MRC macro、dealloc block 釋放、activateServer re-inject；Phase 113 完成——LRU/Prune 閾值統一為 2 + 孤兒偵測；Phase 112 後續修正完成——asyncOnMain crash 修復、delayed dealloc block 釋放、orphan controller prune + client wrapper terminate。基於 IMK 10.9/10.11/10.13/10.14/10.15 六版逆向。詳見 DevPlans/Reqs4LLM/Reqs_0111-0120.md。）
...
> ⚠️ **注意**: 本文檔需要定期更新以反映最新程式碼狀態。最後更新：Phase 106（2026-07-06）。
