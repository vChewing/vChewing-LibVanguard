...
extension HomaTestsRoot {
  ...
  public struct HomaTestsAdvanced: HomaTestSuite {
    ...
    func testCandidateRevolvementWithConsolidation(partialMatch: Bool) async throws {
      ...
      try cases.forEach { candidateCursorType in
        ...
        try (0 ... assembler.length).forEach { pos in
          ...
          var allCandidates = [(
            candidate: Homa.CandidatePairWeighted,
    ...
    func testRevolverOverridesNeighboringExplicitNode_Scenario1() async throws {
      ...
      print(
        "[Scenario1] assembled after first revolve: \(assembler.assembledSentence.map(\.value))"
      )
      ...
      print(
        "[Scenario1] assembled after second revolve: \(assembler.assembledSentence.map(\.value))"
      )
    ...
    func testConsolidationUsesTrueNodeAnchorsOnOverlap() async throws {
      ...
      let logicalCursor = assembler
        .getLogicalCandidateCursorPosition(forCursor: candidateCursorType)
      #expect(throws: Never.self) {
        try assembler.overrideCandidate(
          targetCandidate,
          at: logicalCursor,
