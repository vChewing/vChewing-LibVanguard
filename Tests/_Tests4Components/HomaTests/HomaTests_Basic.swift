  ...
  func testMultipleKeysCartesianProductMergesResults() async throws {
    let assembler = Homa.Assembler(
      gramQuerier: { keyArray in
        ...
        switch keyArray {
        case [.singleKey("shu4")]:
          return [Homa.Gram(keyArray: ["shu4"], current: "樹", previous: nil, probability: -5.0)]
        case [.singleKey("zhi")]:
          return [Homa.Gram(keyArray: ["zhi"], current: "之", previous: nil, probability: -4.9)]
        ...
        default:
          if keyArray.count == 1, case let .multipleKeys(keys) = keyArray[0], Set(keys) == [
            "shu4",
            "zhi",
          ] {
            return [
              Homa.Gram(keyArray: ["shu4"], current: "樹", previous: nil, probability: -5.0),
              Homa.Gram(keyArray: ["zhi"], current: "之", previous: nil, probability: -4.9),
            ]
          }
          if keyArray.count == 2, case let .multipleKeys(keys) = keyArray[0], Set(keys) == [
            "shu4",
            "zhi",
          ], keyArray[1] == .singleKey("zhi") {
            return [
              Homa.Gram(keyArray: ["shu4", "zhi"], current: "樹之", previous: nil, probability: -4.0),
              Homa.Gram(keyArray: ["zhi", "zhi"], current: "之之", previous: nil, probability: -6.0),
            ]
          }
          return []
        }
...
public struct HomaTestsBasic: HomaTestSuite {
  ...
  func testAssemblerQueryGramsPreservesSourceOrderForEqualScorePeers() async throws {
    let assembler = Homa.Assembler(
      gramQuerier: { keyArray in
        ...
        return [
          Homa.Gram(keyArray: flatKeys, current: "，", previous: nil, probability: -9.9),
          Homa.Gram(keyArray: flatKeys, current: "〈", previous: nil, probability: -9.9),
          Homa.Gram(keyArray: flatKeys, current: "《", previous: nil, probability: -9.9),
        ]
      }
  ...
  func testAssemblerQueryGramsPrefersProbabilityOverActualKeyForPartialSingletons() async throws {
    let assembler = Homa.Assembler(
      gramQuerier: { _ in
        [
          Homa.Gram(keyArray: ["shi2"], current: "時", previous: nil, probability: -6),
          Homa.Gram(keyArray: ["shi4"], current: "世", previous: nil, probability: -4),
          Homa.Gram(keyArray: ["shuai1"], current: "衰", previous: nil, probability: -8),
        ]
      }
    )
