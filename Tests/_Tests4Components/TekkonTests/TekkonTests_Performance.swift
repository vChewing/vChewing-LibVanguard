...
struct TekkonPerformanceTests {
  ...
  func testDynamicLayoutPerformance() async throws {
    ...
    for parser in Tekkon.MandarinParser.allCases.filter(\.isDynamic) {
      ...
      print(" -> [Tekkon][(\(parser.nameTag))] \(iterations) iterations in \(timeDeltaStr)s (avg: \(avgTimeStr)s per iteration)")
      
      // 效能期望：每次迭代應該在20ms以內完成（包含12個測試序列）
      #expect(avgTime < 0.02, "Performance regression: \(parser.nameTag) took \(avgTimeStr)s per iteration")
    }
  }
  ...
  func testMemoryOptimization() async throws {
    ...
    // 物件重用應該比重新建立快
    #expect(reuseTime < recreateTime, "Object reuse should be faster than recreation")
  }
